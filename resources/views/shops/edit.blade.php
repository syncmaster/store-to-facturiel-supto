        ...
        <div class="col-xs-12 col-md-6 col-md-push-3">
            <form method="POST">
                ...
                <div class="form-group @if($errors->has('url')) has-error @endif">
                    ...
                    <input type="text" @isset($data['url'])value="{{$data['url']}}"@endisset name="url" class="form-control" id="link">
                    @if($errors->has('url'))<small class="text-danger">{{$errors->first('url')}}</small>@endif
                </div>
                <div class="form-group @if($errors->has('number')) has-error @endif">
                    <label for="number">Api номер:</label>
                    <input type="text" @isset($data['number'])value="{{$data['number']}}"@endisset name="number" class="form-control" id="number">
                    @if($errors->has('number'))<small class="text-danger">{{$errors->first('number')}}</small>@endif
                </div>
                <div class="form-group @if($errors->has('key')) has-error @endif">
                    <label for="key">Api ключ:</label>
                    <input type="text" @isset($data['key'])value="{{$data['key']}}"@endisset name="key" class="form-control" id="key">
